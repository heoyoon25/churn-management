...
with st.sidebar:
    ...
    st.write("데이터 분할:",
             "✅" if st.session_state.X_train is not None else "❌")
    st.write("LR 학습:",
             "✅" if st.session_state.lr_results is not None else "❌")
    st.write("DT 학습:",
             "✅" if st.session_state.dt_results is not None else "❌")
...
# ════════════════════════════════════════════════════════
    ...
    if st.session_state.df is None:
        st.warning("⚠️ 먼저 메인 페이지에서 데이터를 업로드해주세요.")
        st.stop()
    ...
    c2.metric("열 수",       f"{df.shape[1]:,}")
    c3.metric("결측치",      f"{df.isnull().sum().sum():,}")
    c4.metric("수치형 변수", f"{len(df.select_dtypes(include='number').columns)}")
    ...
    if len(num_cols) >= 2:
        ...
        st.info("수치형 변수가 2개 이상이어야 히트맵을 표시할 수 있습니다.")
...
# ════════════════════════════════════════════════════════
    ...
    with st.expander("❓ 결측치 처리", expanded=True):
        ...
        if missing.empty:
            ...
            if st.button("결측치 처리 실행", key="btn_missing"):
                ...
                st.success(f"✅ 완료! 남은 결측치: {tmp.isnull().sum().sum()}")
                st.rerun()

    # 이상치
    with st.expander("⚠️ 이상치 처리 (IQR)", expanded=False):
        num_cols = st.session_state.processed_df.select_dtypes(
            include="number").columns.tolist()
        if not num_cols:
            st.info("수치형 변수 없음")
        else:
            out_cols = st.multiselect("처리할 변수", num_cols,
                                      default=num_cols[:3])
            out_method = st.radio(
                "방법",
                ["IQR 클리핑(상·하한 대체)", "IQR 행 삭제"],
                horizontal=True
            )
            if out_cols:
                preview = []
                for col in out_cols:
                    ...
                    preview.append({"변수": col, "하한": round(lo, 2),
            ...
            if st.button("이상치 처리 실행", key="btn_outlier"):
                ...
                st.session_state.processed_df = tmp
                st.success(f"✅ 완료! 현재 행 수: {len(tmp):,}")
                st.rerun()
    ...
    with st.expander("🔤 원핫 인코딩", expanded=False):
        ...
        if not cat_cols:
            ...
            enc_cols = st.multiselect("인코딩할 변수", cat_cols,
                                      default=cat_cols)
    ...
    with col2:
        ...
        sel_feats = st.multiselect("📌 독립변수 (X)", feat_opts,
                                   default=feat_opts)

    if sel_feats:
        st.info(f"독립변수 **{len(sel_feats)}개** 선택 | "
                f"종속변수: **{target_col}**")
    ...
    col1, col2 = st.columns(2)
    with col1:
        split     = st.radio("Train : Test 비율",
                             ["7 : 3", "8 : 2"], horizontal=True)
    ...
    st.markdown(f"""
    <div style='display:flex;height:30px;border-radius:8px;
    ...
    if st.session_state.X_train is not None:
        st.success(
            f"✅ 분할 완료 | "
            f"Train: {len(st.session_state.X_train):,} / "
...
# ════════════════════════════════════════════════════════
    ...
    if st.button("🚀 Logistic Regression 학습",
                 type="primary", key="btn_lr"):
        with st.spinner("학습 중..."):
            try:
                model = LogisticRegression(
                    C=lr_C, max_iter=lr_iter,
    ...
    if st.session_state.lr_results is not None:
        ...
        with col2:
            ...
            if len(m.classes_) == 2:
                ...
                fig = px.bar(
                    ...
                    template="plotly_white", title="LR 계수"
                )
    ...
    if st.button("🚀 Decision Tree 학습",
                 type="primary", key="btn_dt"):
        with st.spinner("학습 중..."):
            try:
...
# ════════════════════════════════════════════════════════
    ...
    lr_done = st.session_state.lr_results is not None
    dt_done = st.session_state.dt_results is not None
    ...
    if len(y_test.unique()) != 2:
        ...
        if lr_done:
            fpr, tpr, _ = roc_curve(
                y_test, st.session_state.lr_results["y_prob"])
            roc_auc = auc(fpr, tpr)
            fig_roc.add_trace(go.Scatter(
            ...
            auc_rows.append({"모델": "Logistic Regression",
                             "AUC": round(roc_auc, 4)})
        if dt_done:
            fpr, tpr, _ = roc_curve(
                y_test, st.session_state.dt_results["y_prob"])
            roc_auc = auc(fpr, tpr)
            fig_roc.add_trace(go.Scatter(
                x=fpr, y=tpr, mode="lines",
            ...
            auc_rows.append({"모델": "Decision Tree",
                             "AUC": round(roc_auc, 4)})
    ...
    if lr_done:
        ...
        result_df["LR_예측확률"] = \
            st.session_state.lr_results["y_prob"].round(4)
    if dt_done:
        result_df["DT_예측값"]   = st.session_state.dt_results["y_pred"]
        result_df["DT_예측확률"] = \
            st.session_state.dt_results["y_prob"].round(4)
